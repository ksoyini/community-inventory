...
<div class="fieldcontain ${hasErrors(bean: pantryInventoryInstance, field: 'pantryItem', 'error')} required">
	...
	<g:select id="pantryItem" name="pantryItem.id" from="${scott.kellie.PantryItem.list()}" optionKey="id" required="" value="${pantryInventoryInstance?.pantryItem?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: pantryInventoryInstance, field: 'quantity', 'error')} required">
	...
	<g:field name="quantity" type="number" min="0" value="${pantryInventoryInstance.quantity}" required=""/>
</div>
